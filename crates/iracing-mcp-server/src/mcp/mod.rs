...
fn default_resolve_limit() -> usize { 3 }
...
pub async fn handle_request(state: &ServerState, req: JsonRpcRequest) -> JsonRpcResponse {
    ...
    match req.method.as_str() {
        ...
        "tools/list" => ok(req.id, json!({ "tools": tool_descriptors() })),
...
async fn tools_call(state: &ServerState, id: Option<Value>, params: Value) -> JsonRpcResponse {
    ...
    match name {
        ...
        "replay_get_state" => match state.adapter.get_replay_state().await {
            Ok(replay_state) => tool_ok(id, replay_state),
            Err(error) => tool_err(id, error_code(&error), &error.to_string()),
        },
        "replay_set_playback" => replay_set_playback(state, id, params).await,
        "replay_seek_session_time" => replay_seek_session_time(state, id, params).await,
        "replay_seek_frame" => replay_seek_frame(state, id, params).await,
        "replay_search_event" => replay_search_event(state, id, params).await,
        "replay_show_window" => replay_show_window(state, id, params).await,
        "camera_focus" => camera_focus(state, id, params).await,
        "camera_set_state" => camera_set_state(state, id, params).await,
        "get_weekend_info" => match state.adapter.get_weekend_info().await {
        ...
        "get_camera_groups" => match state.adapter.get_camera_groups().await {
        ...
        "resolve_driver" => {
            ...
            match state.adapter.resolve_driver(&args.query, args.limit).await {
...
async fn replay_set_playback(
    ...
    let before = match state.adapter.get_replay_state().await {
        Ok(before) => before,
    ...
    loop {
        match state.adapter.get_replay_state().await {
            Ok(current) => {
                let verified = verify_playback_state(
                    &before,
                    &current,
                    &args,
                    &mut pause_candidate_frame,
                );
...
async fn replay_seek_frame(
    ...
    if let Err(error) = state.adapter.replay_seek_frame(args.mode, args.frame).await {
        return tool_err(id, error_code(&error), &error.to_string());
    }
...
async fn replay_search_event(
    ...
    if let Err(error) = state.adapter.replay_search_event(args.mode).await {
        return tool_err(id, error_code(&error), &error.to_string());
    }
...
async fn replay_show_window(
    ...
    if args.start_time_ms < 0 || args.focus_car_idx < 0 || args.session_num < 0 {
        return tool_err(id, "invalid_arguments", "sessionNum, startTimeMs, and focusCarIdx must be >= 0");
    }
    ...
    if let Err(error) = state.adapter.set_replay_playback(args.speed, false).await {
        return tool_err(id, error_code(&error), &error.to_string());
    }
    ...
    let final_state = match state.adapter.get_replay_state().await {
        Ok(current) => current,
        Err(error) => return tool_err(id, error_code(&error), &error.to_string()),
    };
    let all_verified = steps
        .iter()
        .all(|step| step.get("verified").and_then(Value::as_bool).unwrap_or(false));
...
fn verify_search_event_state(mode: ReplaySearchMode, before: &ReplayState, current: &ReplayState) -> bool {
    match mode {
...
fn apply_camera_state_updates(base: i32, args: &CameraSetStateArgs) -> i32 {
    ...
    set_bit(&mut state, USE_AUTO_SHOT_SELECTION, args.use_auto_shot_selection);
...
fn verify_playback_state(
    ...
    if current.replay_play_speed != args.speed || current.replay_play_slow_motion != args.slow_motion {
        return false;
    }
