...
impl IracingAdapter for StubAdapter {
    ...
    async fn camera_focus(
        ...
        if let Some(g) = group_number { s.cam_group_number = g; }
        if let Some(c) = camera_number { s.cam_camera_number = c; }
        Ok(())
    }
    ...
    async fn get_camera_groups(&self) -> Result<CameraGroupList, AdapterError> {
        let groups = vec![
            CameraGroup {
                ...
                is_scenic: false,
                cameras: vec![
                    CameraEntry { camera_num: 0, camera_name: "Chase".to_string() },
                    CameraEntry { camera_num: 1, camera_name: "Cockpit".to_string() },
                ],
            },
            CameraGroup {
                group_num: 2,
                ...
                is_scenic: true,
                cameras: vec![
                    CameraEntry { camera_num: 0, camera_name: "Blimp".to_string() },
                ],
            },
        ];
    ...
    async fn get_standings(&self, _session_num: Option<i32>) -> Result<Standings, AdapterError> {
        let positions = vec![
            SessionPosition {
                position: 1, class_position: 1, car_idx: 7, lap: 5, laps_complete: 4,
                fastest_lap: 3, fastest_time: 92.5, last_time: 93.1, incidents: 0,
                reason_out: "Running".to_string(),
            },
            SessionPosition {
                position: 2, class_position: 2, car_idx: 0, lap: 5, laps_complete: 4,
                fastest_lap: 2, fastest_time: 93.2, last_time: 94.0, incidents: 2,
                reason_out: "Running".to_string(),
            },
        ];
