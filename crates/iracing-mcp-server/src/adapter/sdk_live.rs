...
impl SdkAdapter {
    ...
    fn set_replay_playback_sync(&self, speed: i32, slow_motion: bool) -> Result<(), AdapterError> {
        ...
        debug!("set_replay_playback: sending ReplaySetPlaySpeed speed={} slow_motion={}", speed, slow_motion);
    ...
    fn replay_search_event_sync(&self, mode: ReplaySearchMode) -> Result<(), AdapterError> {
        ...
        debug!("replay_search_event: sending ReplaySearch mode={:?} code={}", mode, mode_code);
    ...
    fn camera_set_state_sync(&self, state_bits: i32) -> Result<(), AdapterError> {
        ...
        debug!("camera_set_state: sending CamSetState state_bits={}", state_bits);
...
impl IracingAdapter for SdkAdapter {
    ...
    async fn get_weekend_info(&self) -> Result<WeekendInfo, AdapterError> {
        ...
        let wi_f = |k: &str| -> f64 {
            root.get("WeekendInfo")
                ...
                .and_then(|v| {
                    v.as_f64().or_else(|| {
                        v.as_str().and_then(|s| s.trim_end_matches(" km").parse().ok())
                    })
                })
                .unwrap_or(0.0)
        };
        ...
        let weather_f = |k: &str| -> f64 {
            root.get("WeekendInfo")
                .and_then(|v| v.get("WeatherParams").or_else(|| Some(v)))
                .and_then(|v| v.get(k))
                .and_then(|v| v.as_f64())
                .unwrap_or(0.0)
        };
    ...
    async fn get_roster(
        ...
        let i64_field = |d: &YamlValue, k: &str| -> i64 {
            d.get(k).and_then(|v| v.as_i64()).unwrap_or(0)
        };
        let b_field = |d: &YamlValue, k: &str| -> bool {
            d.get(k).and_then(|v| v.as_i64()).map(|n| n != 0).unwrap_or(false)
        };
                    ...
                    .map(|cams| {
                        cams.iter()
                            .filter_map(|c| {
                                ...
                                Some(CameraEntry { camera_num, camera_name })
                            })
                            .collect()
                    })
                    .unwrap_or_default();
                Some(CameraGroup { group_num, group_name, is_scenic, cameras })
            })
            .collect();
            ...
            .map(|pos| {
                pos.iter()
                    .filter_map(|p| {
                        ...
                        let f_f = |k: &str| -> f64 {
                            p.get(k).and_then(|v| v.as_f64()).unwrap_or(-1.0)
                        };
    ...
    async fn resolve_driver(
        ...
        Ok(ResolveDriverResult { best_match, candidates: scored })
    }
}
...
fn find_car_number_for_car_idx(session_yaml: &str, car_idx: i32) -> Result<String, AdapterError> {
    ...
    driver
        ...
        .ok_or_else(|| AdapterError::SessionInfo("DriverInfo.Drivers[].CarNumber missing".to_string()))
}
...
fn send_broadcast_message_2(message: i32, var1: i32, var2: i32) -> Result<(), AdapterError> {
    ...
    unsafe {
        ...
        debug!("send_broadcast_message_2: registered message_id={}", message_id);

        let success = SendNotifyMessageW(
            HWND_BROADCAST,
            message_id,
            wparam,
            lparam,
...
fn send_broadcast_message_3(
    ...
    Err(AdapterError::Broadcast(
        "iRacing broadcast messages are only available on Windows".to_string(),
    ))
}
