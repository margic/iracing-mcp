...
async fn live_mcp_camera_focus_switches_target_and_camera() {
    ...
    assert_eq!(restored["observed"]["camCarIdx"], Value::from(current_car_idx));
}
...
async fn live_mcp_camera_set_state_verifies_and_restores() {
    ...
    let before_state = before["camCameraState"].as_i64().expect("camCameraState as i64") as i32;
    let set = call_tool(
        app.clone(),
        "camera_set_state",
        json!({
    ...
    let observed_state = set["observed"]["camCameraState"].as_i64().expect("camCameraState as i64") as i32;
...
async fn live_mcp_replay_search_to_end_verifies_after_seek_back() {
    ...
    let session_num = before["replaySessionNum"].as_i64().expect("replaySessionNum as i64") as i32;
    let current_time_ms = (before["replaySessionTime"].as_f64().expect("replaySessionTime as f64") * 1000.0)
        .round() as i32;
    ...
    let payload = call_tool_payload(app.clone(), "replay_search_event", json!({ "mode": "to_end" })).await;
    assert_eq!(payload["ok"], Value::Bool(true), "unexpected payload: {payload}");
    assert_eq!(payload["data"]["verified"], Value::Bool(true), "unexpected payload: {payload}");
}
...
async fn live_mcp_replay_set_playback_verifies_and_restores() {
    ...
    let original_speed = before["replayPlaySpeed"].as_i64().expect("replayPlaySpeed as i64") as i32;
    let original_slow_motion = before["replayPlaySlowMotion"].as_bool().expect("replayPlaySlowMotion as bool");
    let session_num = before["replaySessionNum"].as_i64().expect("replaySessionNum as i64") as i32;
    let current_time_ms = (before["replaySessionTime"].as_f64().expect("replaySessionTime as f64") * 1000.0)
        .round() as i32;
    ...
    assert_eq!(
        ...
        Value::from(if original_speed == 0 { 1 } else { original_speed })
    );
}
...
async fn call_tool(app: axum::Router, name: &str, arguments: Value) -> Value {
    ...
    payload.get("data").cloned().unwrap_or_else(|| {
        panic!("tool {name} returned ok=true but missing data field: {payload}")
    })
}
...
async fn call_tool_payload(app: axum::Router, name: &str, arguments: Value) -> Value {
    ...
    json
        ...
        .cloned()
        .unwrap_or_else(|| {
            panic!("tool {name} returned unexpected response envelope: {json}")
        })
}
...
async fn choose_alternate_car_idx(app: &axum::Router, current_car_idx: i32) -> Option<i32> {
    ...
    entries
        .iter()
        .filter_map(|entry| entry.get("carIdx").and_then(Value::as_i64).map(|v| v as i32))
