...
impl adapter::IracingAdapter for ScriptedAdapter {
    ...
    async fn replay_seek_frame(
        ...
        let target = match mode {
            ...
            adapter::ReplaySeekFrameMode::Current => inner.current.replay_frame_num.saturating_add(frame),
            adapter::ReplaySeekFrameMode::End => inner.current.replay_frame_num_end.saturating_sub(frame),
        };
    ...
    async fn get_camera_groups(
        &self,
    ) -> Result<adapter::CameraGroupList, adapter::AdapterError> {
...
async fn replay_search_to_end_verifies_after_delayed_polls() {
    ...
    let adapter = Arc::new(
        ScriptedAdapter::new(initial.clone()).with_script(
            "replay_search_event",
            vec![
                adapter::ReplayState {
    ...
    assert_eq!(payload["data"]["observed"]["replayFrameNum"], Value::from(11_997));
}
...
async fn replay_show_window_timeout_reports_step_level_verification() {
    ...
    assert_eq!(payload["error"]["code"], Value::String("timeout".to_string()));
    assert_eq!(payload["data"]["verified"], Value::Bool(false));
    assert_eq!(payload["data"]["steps"][0]["tool"], Value::String("replay_seek_session_time".to_string()));
    assert_eq!(payload["data"]["steps"][0]["verified"], Value::Bool(true));
    assert_eq!(payload["data"]["steps"][1]["tool"], Value::String("camera_focus".to_string()));
    assert_eq!(payload["data"]["steps"][1]["verified"], Value::Bool(true));
    assert_eq!(payload["data"]["steps"][2]["tool"], Value::String("replay_set_playback".to_string()));
    assert_eq!(payload["data"]["steps"][2]["verified"], Value::Bool(true));
    assert_eq!(payload["data"]["steps"][3]["tool"], Value::String("replay_pause_at_end".to_string()));
    assert_eq!(payload["data"]["steps"][3]["verified"], Value::Bool(false));
    assert_eq!(payload["data"]["steps"][3]["reachedEnd"], Value::Bool(false));
}

#[tokio::test]
async fn replay_commands_return_wrong_mode_when_in_car() {
    let adapter = Arc::new(ScriptedAdapter::new(adapter::ReplayState {
        is_on_track: true,
        ..base_replay_state()
    }));
    let app = build_app(adapter);
    ...
    assert_eq!(payload["error"]["code"], Value::String("wrong_mode".to_string()));
}

#[tokio::test]
async fn camera_focus_verifies_car_target_even_if_camera_slot_changes() {
    let initial = base_replay_state();
    let adapter = Arc::new(ScriptedAdapter::new(initial));
    let app = build_app(adapter);
...
async fn camera_set_state_verifies_requested_bits() {
    ...
    assert_eq!(payload["ok"], Value::Bool(true));
    assert_eq!(payload["data"]["verified"], Value::Bool(true));
    let observed_state = payload["data"]["observed"]["camCameraState"].as_i64().expect("camCameraState as i64") as i32;
