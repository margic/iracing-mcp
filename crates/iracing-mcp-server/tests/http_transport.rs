...
async fn http_mcp_m1_read_tools_work() {
    ...
    assert_eq!(wi["trackLengthKm"], Value::Number(serde_json::Number::from_f64(5.0).unwrap()));

    // get_roster
    let roster = mcp_call("get_roster", json!({})).await;
    assert_eq!(roster["count"], Value::Number(2.into()));
    assert_eq!(roster["entries"][0]["userName"], Value::String("Alice Driver".into()));
    ...
    assert_eq!(rd["bestMatch"]["matchReason"], Value::String("exact".into()));
